...
#define MRF_FSM_RESET()     do{ mrf_write_short(RFCTL, 0x04); mrf_write_short(RFCTL, 0x00); wait_us(2000);} while(0)
...
void rf_init(RF_RX_INFO *pRRI, uint8_t channel, uint16_t panId, uint16_t myAddr)
{
  ...
  // rf_tx_power(20); // Vuk: this is set in radio initialization
	rf_set_rx_mode(0); // Vuk: 2 for ERROR reception mode
...
void rf_test_mode(void)
{
    //mrf_write_long(TESTMODE, 0x08 | 5);    // Put into single-tone test mode
		mrf_write_long(TESTMODE, 0x0D); // Put into single-tone test mode
}

void rf_data_mode(void)
{
	#ifdef MRF24J40MA
		mrf_write_long(TESTMODE, 0x08);        // Put back into default mode MRF24J40MA
	#endif
	#ifdef MRF24J40MD	
		mrf_write_long(TESTMODE, 0x08);        // Put back into default mode MRF24J40MD
	#endif
}
